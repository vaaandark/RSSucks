...
pub struct Entry {
    ...
    /// If not provided or invalid, it is `None`.
    pub updated: Option<DateTime<Utc>>,
    /// The IDs of entries which belong to this folder.
    articles: BTreeSet<ArticleUuid>,
    /// A unique identifier for this feed, generated by [`feed_rs`]
    hash: String,
...
impl Entry {
    ...
    fn add_article(&mut self, article_id: ArticleUuid) {
...
pub struct Feed {
    ...
    articles_map: BTreeMap<ArticleUuid, Article>,
}
...
impl Feed {
    ...
    pub fn add_article(&mut self, id: ArticleUuid, article: Article) {
        self.articles_map.insert(id, article);
    }

    #[allow(unused)]
    pub fn try_update_articles_blocking(&mut self) -> Result<()> {
        ...
        let article_pairs = self
            .get_all_entries()
            .map(|entry| {
                let mut entry = entry.borrow_mut();
                let url = entry.xml_url.as_str();
                let feed = feed_rs::parser::parse(client.get(url).send()?.text()?.as_bytes())?;
                // Need updating.
                Ok(if feed.updated != entry.updated || feed.id != entry.hash {
                    entry.hash = feed.id.to_owned();
                    entry.updated = feed.updated;
                    feed.entries
                        .iter()
                        .filter_map(|item| {
                            let article_id =
                                ArticleUuid::new(entry.updated, &entry.uuid, item.id.clone());
                            if !entry.articles.contains(&article_id) {
                                let article = Article::from(item.to_owned());
                                entry.add_article(article_id.clone());
                                Some((article_id, article))
                            } else {
                                None
                            }
                        })
            ...
            .flatten()
            .collect::<Vec<_>>();
        article_pairs.into_iter().for_each(|(id, article)| {
            self.add_article(id, article);
        });
        Ok(())
    }
}
...
mod test {
    ...
    fn feed_update() {
        ...
        feed.try_update_articles_blocking().unwrap();
        let fetched_article = feed.articles_map.len();
        println!("{}", fetched_article);
    }
}
