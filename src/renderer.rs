...
fn richtext_generator(text: &str, dom_stack: &Vec<ElementType<'_>>) -> Result<egui::RichText> {
    let richtext =
        dom_stack.iter().fold(
...
pub fn render_article_to_detail_component(
    channel: &str,
    author: Option<&str>,
    title: &str,
    link: &str,
    ...
    ui.group(|ui| {
        ui.group(|ui| {
            ...
            ui.hyperlink_to(RichText::new(title).size(32.0).strong(), link);
            ui.horizontal_wrapped(|ui| {
                ui.spacing_mut().item_spacing = egui::vec2(0.0, 0.0);
                if let Some(author) = author {
                    ui.label(RichText::new(author).size(AUTHOR_SIZE));
                    ui.label(RichText::new(" @ ").size(AUTHOR_SIZE));
                    ui.label(RichText::new(channel).size(AUTHOR_SIZE));
                } else {
                    ui.label(RichText::new(channel).size(AUTHOR_SIZE));
                }
                ui.label(RichText::new("\t").size(AUTHOR_SIZE));
                ui.label(RichText::new(time).size(AUTHOR_SIZE));
            });
        });
        ui.group(|ui| {
